...
const AnalogDelay = () => {
  ...
  const dly = useRef(pb.effects.find(fx => fx instanceof Delay));
  const [on, setOn] = useState(false)
  const [delay, setDelay] = useState(0.2) // in seconds
  const [feedback, setFeedback] = useState(0.2) // 20%
  const [filter, setFilter] = useState(2000) // Lowpass freq in Hz
  const [dry, setDry] = useState(1) // 100%
  const [wet, setWet] = useState(0.2) // 20%
  ...
  const setDelayTime = (e) => {
    ...
    setDelay(time)
    dly.current.delay.delayTime.linearRampToValueAtTime(time, pb.ctx.currentTime + 0.2)
  }
